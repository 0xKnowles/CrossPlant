...
- Plants Stuck Asleep Forever on X4: X4 has no RTC chip, and the automatic clock sync on WiFi connect skipped entirely on devices without one — so X4's system clock never got set from NTP, `time(nullptr)` stayed invalid or stuck at whatever time the pet state was last saved, and plants could get permanently stuck reporting "asleep" (blocking all care actions and growth) regardless of the actual time of day. X4 now syncs its system clock from NTP every time WiFi connects (X3, which has a battery-backed RTC, keeps its existing sync-once behavior).
- Clock-Dependent Features Missing on X4: A whole set of features checked for RTC *hardware* rather than "do we know the time", so they silently stayed off on X4 even after its clock was synced. Date-stamped reading stats and stat-date editing, timestamps on exported clippings, correct file modification dates on the SD card, the header/status-bar clock and date, the daily-average reading stat, and automatic reading-stats backups all now work on X4 once its clock is set. "Sync Date/Time Now" (Settings > System > Device) is also no longer hidden on X4 — previously the one control that could establish a clock was unavailable on the devices that needed it most.
- Clock Reset to 1970 After Power Off on X4: Without an RTC, X4 lost the time on every full power cycle and started from the Unix epoch until the next WiFi connection. The last known time is now saved before sleep and restored at boot, so the device starts with an approximately-correct clock (corrected exactly by the next NTP sync) instead of an invalid one.
- Menu Text Running Off Screen: Plant action names, Dew Shop item lines, and daily-quest titles were drawn without any width limit, so longer labels — common in several of the 26 supported languages, and on X4's narrower screen — overflowed their panels and ran off the display. These now truncate with an ellipsis, and shop item descriptions wrap to a second line instead of being cut off.
- My Plants Screen Cut Off on X4: The "Plant Actions" list (Water Plant, Fertilize, Buy Fertilizer, etc.) was rendering past the right edge of the screen on X4, making most actions unreadable/unselectable. The status card's column width was accidentally wider on X4's narrower screen than on X3's wider one, leaving almost no room for the actions column; rebalanced so both columns fit.
- Sleep Screen Logo Rendered Sideways: Fixed the plant/seed logo on the plain Light/Dark sleep screens (and the no-plant fallback on the My Plants sleep screen) rendering rotated 90°. It was drawn with a raw bitmap blit that only repositions for the screen's orientation without rotating the pixel content itself; switched it to the same per-pixel baked-image draw used everywhere else this art appears, which is fully orientation-aware.
